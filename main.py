...
@lru_cache(maxsize=200)  # Increased cache size for better reuse
def fetch_url(url):
    headers = {
...
def list_videos(VIDEOS):
    ...
    content = fetch_url(url)[:20000]  # Limit the content scanned by regex
    links = list(
        {
...
if __name__ == "__main__":
    list_videos(VIDEOS)  # Populate videos synchronously
    router(sys.argv[2][1:])
